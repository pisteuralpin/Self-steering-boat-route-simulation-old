...
print("Currents max step : {} m/s".format(round(vmax,2)))
...
print("Initial heading : {}°".format(round(360 - ini_dir/np.pi*180,1)))         # Print initial heading
...
while (0 <= int(boat_pos2[0, -1:]) < size[0]) and \
      ...
      (np.sqrt((boat_pos2[0, -1:]-goal_pos[0])**2 + (boat_pos2[1, -1:]-goal_pos[1])**2) > 1) and \
       i < len(currents_map): ## JPC
